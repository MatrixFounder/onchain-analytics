...
- *(Дополнение 2026-07-25 — решение владельца; РАСПРОСТРАНЯЕТ дополнение 2026-07-20 со снапшоттера
  на весь автономный контур, снимает противоречие в ROADMAP/functional-architecture)*
  **Требование: система автономна в части анализа данных.** Кроны и push-уведомления обязаны
  работать без открытой сессии Claude Code — значит always-on слой это **n8n + Postgres**, и
  «поглощение снапшоттера движком» в M3 **отменяется**.

  Что из этого следует:
  - **Снапшоттер остаётся на n8n**, пишет в Supabase Postgres (схема `onchain`). Это не легаси —
    это принятая архитектура для профиля «выделенный сервер».
  - **Планировщик сигналов M3 — тоже n8n**, не in-process `croner`. Тот же драйвер, что и в
    дополнении 2026-07-20, только теперь применённый к правилам, а не только к замерам:
    MCP-сервер живёт stdio-процессом ровно столько, сколько открыта сессия хоста, поэтому
    расписание внутри него по определению не автономно.
  - **Движок остаётся pull-стороной**: канонические способности, нормализация, budget-guard, кеш.
    Историю он читает из того же Postgres read-only (адаптер `pg-history`, построен в M1).
  - `croner` + durable job-log сохраняются как референс для локального/embedded-профиля — ровно
    так же, как в дополнении 2026-07-20.

  Открытый вопрос, который это решение ПОРОЖДАЕТ (не решает): каким интерфейсом n8n вызывает
  способности движка — stdio MCP по сети недоступен. См. OQ-M3-1 в
  [open-questions.md](../architectures/open-questions.md).
...
- *(Дополнение 2026-07-25 — следствие D8-дополнения того же дня)* Для профиля «выделенный сервер»
  push-уведомления автономного контура идут **через n8n** (Telegram-credential «Onchain bot», уже
  используется в `onchain-error-alert`), а не через собственного `grammY`-бота внутри движка: бот
  в stdio-процессе не автономен по тому же признаку, что и планировщик. `grammY` + абстракция
  `Notifier` остаются для локального/embedded-профиля и для уведомлений, инициированных самим
  движком в ответ на pull-запрос.
