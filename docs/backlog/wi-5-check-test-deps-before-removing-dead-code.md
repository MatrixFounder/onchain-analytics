...
> **DONE 2026-07-28.** Encoded in `code-review-checklist` §6, directly under _Law of Minimalism_ —
> the line that generates these findings: grep the symbol across `test/` before acting, and report
> "unreachable **and** covered by test X" rather than "dead, delete". A finding may be right about
> the smell and wrong about the fix. Mirrored as a rationalization-table row in
> `developer-guidelines`, since the developer is who acts on it.
