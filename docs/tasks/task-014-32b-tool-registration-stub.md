...
- [ ] Оба `ToolSpec` зарегистрированы; схемы `.strict()`; сьюта зелёная на заглушках
- [ ] Восемь инвентарных каналов пройдены для каждого тула
- [ ] Непочиненный прогон исполнен первым, измеренный список упавших утверждений записан в задаче
- [ ] Измеренный список сверен с перечнем раздела «Гейты вне восьми инвентарных каналов»: упавшее и
      не названное дописано, названное и не упавшее вычеркнуто с причиной
- [ ] Каждое утверждение измеренного списка зелёное
- [ ] Заглушка отвечает типизированным отказом, называющим задачу-снимающую
- [ ] `pool.info` и `token.pools` названы в `CAPABILITY_EXCLUSIONS`; причина называет снимающую
      задачу
- [ ] AC-2 (частично): снимок `tools/list` двигается один раз, обоснование в коммите
- [ ] AC-11 (частично): ни одно поле обеих схем не порождает хост, URL или эндпоинт
- [ ] AC-13 (частично): строка `token.pools` несёт `shareable`
- [ ] Греп по трём числам прогнан; каждое найденное вхождение обработано по своей форме, и
      повторный прогон не находит ни одного вхождения старого числа в настоящем времени
- [ ] Гейтованная строка отношения ENFORCEMENT (`capability-manifest.ts:95`) читает `27 of 27`
- [ ] Счёт способностей и маршрутов приведён к замеру: 27 способностей, 28 маршрутов — не к «24 и 25»
- [ ] Все числа тулов в настоящем времени обновлены; шаблоны числительных принимают дефисную и
      двухсловную форму вместе с обеими формами существительного; **оба** якоря областей WI-48
      переякорены
- [ ] `ROADMAP.md` несёт грамматически верное «Двадцать два тула», и гейт на нём зелёный
- [ ] Три области перечисления имён содержат оба тула, узел C4 — в своей форме без префикса
- [ ] Оба читателя замороженной TTL-таблицы несут именованное исключение, а не сфабрикованную строку
      «до переезда»
- [ ] Таблица дедлайнов `system-architecture.md:2387` перечисляет `token.pools`
