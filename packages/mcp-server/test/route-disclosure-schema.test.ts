...
describe('the residual this gate does NOT close, measured rather than assumed', () => {
  it('names the tools that publish output without validating it against their own schema', () => {
    ...
    //
    // **Both TEMPORARY entries this list carried are gone.** `pool-info.ts` left in task 014-32c and
    // `token-pools.ts` in 014-32d, each in the commit that replaced its stub with a handler
    // validating against its own output schema. Distinguishing an interval entry from a permanent
    // one is what made that possible: the two remaining entries are permanent by design, and every
    // interval entry named the task that removes it, so neither outlived its reason.
