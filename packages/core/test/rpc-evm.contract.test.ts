...
describe('rpc-evm adapter (contract, R-16/R-17 backend, OQ-1)', () => {
  ...
  it('bounds an oversized raw response inside a normalize() error message to a truncated, fixed-size string (post-M1 polish, fix 5)', () => {
    const oversizedResult = 'z'.repeat(50_000);
    let thrown: Error | undefined;
    try {
      adapter.normalize('wallet.balances.native', {
        chain: 'ethereum',
        address: ADDRESS,
        raw: { jsonrpc: '2.0', id: 1, result: oversizedResult },
      });
    } catch (error) {
    ...
    expect(thrown!.message).toContain('rpc-evm.normalize: invalid balance hex in "result":');
