...
- **TASK-008 (трек A, `dune-tasks.md` §A-2): ✅ выполнен 2026-07-29, отревьюен `/vdd-multi` дважды.** Бесплатный ярус Blockscout: **мёртвая `token.holders`** (маршрут вёл на заглушку `dune`, покрывавшую ноль сетей, пока `onchain_list_chains` её рекламировал) перенацелена на реальный источник, а **`entity.labels` снята с платного Nansen** — он остаётся фолбэком, так что кредит тратится только там, где бесплатный источник не смог. Покрытие выведено генератором из запиненной evidence по числовому `chain_id` (39 сетей из 53 вендорских мейннетов; **никогда** по полю `ecosystem` — оно читается как семейство сетей и им не является).
  ...
  - **Открыто владельцу:** лимитер `{5,5}` против измеренного потолка ≈625 вызовов/сутки (значение предписано самим R-73(b)); `token.holders` рекламируется на 39 сетях, но её не резолвит ни один MCP-тул (тул отложен границами задачи); доверие к провайдеру и цена free-first — в OQ-4.
