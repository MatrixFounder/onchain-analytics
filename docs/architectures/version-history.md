...
- 2026-08-12, **v4.11** — T-014 `http-transport-auth-perimeter-profiles-shared-limiter`:
  **design phase, ahead of code.** Five sections written against TASK-014 (32 requirements, 49
  acceptance criteria). In §4.5, eight persistent tables are added (identity, the shared vendor
  limiter, the per-request trace T-015 charges from, operational records). In §7.5, the project's
  FIRST inbound trust boundary is designed — until now §7 covered only the outbound side. In §3.4,
  transport selection, the session manager, principal threading and the shared limiter are added,
  plus the storage axis and its per-dialect atomicity. In §5.4, the wire contract is defined, and in
  §5.1.7 the tool that closes L-15. In §10, "dev only, local" is replaced with deployment profiles
  and a settings classification.
  ...
  **Three standing constraints of §1.2 were narrowed in scope, not removed** — cache in Postgres,
  writes to the snapshotter's tables, cron inside the server. Each was narrowed because its original
  justification named the local stdio process, and each was verified against that justification by
  the architecture review. **Review: BLOCKING on the first round** (2 BLOCKING, 7 MAJOR, 5 MINOR;
  `docs/reviews/architecture-t014-review.md`). R-21 was absent while one section asserted the tool
  set does not change. The network profile had DDL for a cache and budget layer no component read,
  with the money gate's atomicity still stated as SQLite's. Both fixed in the second pass.
...
- 2026-08-05, **v4.10** — T-013 `series-merge-and-history-tool`: **design phase, ahead of code.**
  ...
  - **The merge mechanism (ADR-002 D5/D6) gets its activation model, its conflict rank, and its
    policy-evaluation point.** These are the three questions `docs/TASK.md` §6 left open
    (`OQ-T013-2`/`3`/`4`). They are closed in [open-questions.md](open-questions.md) and reasoned in
    full in [system-architecture.md](system-architecture.md) "Merge mechanism". (1) Activation needs
    TWO gates, not one: manifest `mergeable` eligibility (R-159, already decided) PLUS a new,
    independent `CapabilityRoute.merge?: boolean`. The reason is that D5's text names the route's
    own descriptor and UC-20 names an act the route performs, which manifest-only activation cannot
    construct. A capability with more than one route is explicitly OUT OF SCOPE, not silently
    solved. (2) The compiled conflict rank reuses the route's existing `adapterIds` order rather
    than a new table or `.trust`/Postgres, narrowly and provisionally, pending T-016's real per-row
    trust axis. It is now ENFORCED, not documented alone: a new construction-time assertion requires
    every `merge: true` participant to be `tier: 'free'`. That closes the hazard of a paid,
    more-authoritative participant silently losing every conflict by sitting last in spend order.
    (3) `policy` is evaluated per-participant, the same predicate the non-merge path already applies
    to cache hits, preserving H-1 rather than replacing it with a whole-array reading. One
    divergence is stated. A merge route where everyone answers and nobody satisfies never falls back
    to a single participant's raw answer the way the non-merge path does, since that would silently
    un-merge the response.
  - **R-164's four-branch outcome contract layers on top of the three existing failure types, not
    ...
    the first satisfying answer. The deadline (OD-4) stays a PRECONDITION that pre-empts all four
    branches, unchanged from the non-merge contract. OD-4 covers all THREE sites that already throw
    `CapabilityDeadlineExceededError`: the per-participant pre-check, an in-flight `fetch()` cut off
    by the ceiling, and the caller's own already-expired deadline at entry. The terminal wall-clock
  ...
  - **`CapabilityResolution` gains three optional fields** (`sources`, `missingSources`,
    ...
    **`sources` means CONTRIBUTORS** (participants whose points are present in `result`). This was
    corrected after review from an earlier "everyone who answered" reading that would have
    attributed a merged payload to a participant that returned nothing of its own.
  - **The 14th tool, `onchain_dash_platform_history`** (interfaces.md §5.1.6), publishes both
    ...
    `servedCapabilities?: readonly string[]`, carries both. Three readers (`eval/capabilities.mjs`,
    `docs-counts.test.ts`, `tool-spec.test.ts`) need a real, named, small behaviour change —
    enumerated in system-architecture.md, not left as "the rest is Development's problem".
  - Dedup by `(metric, asset, HOUR of ts)` (bucketed 2026-08-07, R-161(e) — raw `ts` could never
    collide between the two real participants) never compares `value_raw` numerically. Walking
    contributors in RANK order and inserting into a map only when the key is absent picks the winner
    by construction. No value comparison exists to route through `Number(...)` in the first place
    (R-167).
  - The merged result is never cached as a unit (D5's invariant, the one ADR-002 marks critical).
    Nothing about the merge walk is a new `cache.set()` call; every per-adapter cache read/write is
    the SAME code the non-merge path already runs.
  - **Baseline corrected after review — 1538** (1206 core + 332 mcp-server), not 1473. "No source
    file moved" is still true of T-013 itself, but 1473 is v4.9.3's figure. WI-34…WI-37 landed on
    top of it with **no version-history entry recording the delta**, and carrying 1473 forward here
...
test docs-counts` failed. The fix is that §5.1.6 now carries NO anchor; the anchors land in the
    same commit that registers the `ToolSpec`. Verified green (7/7) after the fix.
  - Round 2 also named TWO more `servedCapabilities` readers that fail SILENTLY
    ...
    reader if left unchanged. Round 2 corrected `data-model.md`'s `source` rule, which stated only
    the all-zero-contributor fallback and not the primary "highest-ranked contributor" rule. It
    extended the `limit`-truncation disclosure to `series:'shielded_pool'` (`pg-history`'s 100-row
    cap applies to BOTH capabilities, not only `platform_metrics`). And it scoped
...
- 2026-08-05, **v4.9.3** — **OQ-T012-6 resolved by the owner: Reading A, with two conditions.** The
  ...
  **The answer is returned, not thrown.** In that branch the deadline prevented nothing and aborted
  nothing. The time, and on a paid route the credit, are already spent, so discarding the result
  does not recover them. It converts complete data into "retry later", and the retry walks the same
  route to the same answer. Whether a late-but-complete answer is worth anything is the caller's
  judgement, because the caller set the deadline. It is also the reversible choice: throwing can be
  added later, while subordinating H-1 to the clock destroys a property that cost a full adversarial
  iteration.
...
- 2026-08-05, **v4.9.2** — T-012 adversarial cycle 3 and the fix pass that followed it. The cycle
  ...
  - **Cycle 2's `_meta.budget` fix was incomplete on the branch it was written for**, and two
    ...
    **cache hit** after the walk entered and paid `nansen`. So the response said `'hit'`, carried no
    ...
    (`budget-meta.ts`). The load-bearing invariant behind that simplification ("the answering
...
floor` is arithmetic done BEFORE the sleep, and a timer is only guaranteed not to fire early —
  ...
  - **A key-in-path RPC endpoint is refused where it is admitted, not where it is printed.**
    ...
    did — is corrected.
  - **The vendor-text surface is measured instead of assumed bounded** (security.md §7.2.2):
    per-field and per-row caps existed, and nobody had multiplied them out. The measured surface is
    < 17.5 KB per entity, 400 entries per nansen response (the two vendor arrays are sliced
    independently), and 512 KiB per blockscout call by transport. Those figures were produced by
    feeding oversized input through `normalize()` and weighing the output.
...
- 2026-08-05, **v4.9.1** — T-012 adversarial cycle 2: nine findings, two of them live defects, the
  ...
  **Prototype-named capabilities and policy kinds are rejected** (`Object.hasOwn`, plus a
  ...
  bounded and the rest carry a declared ceiling. That is sanctioned by R-140e, tracked as WI-37, and
  no longer described as if it cut anything. That last record is **gated rather than written**:
  every manifest row carries its own ENFORCED/DECLARED marker. `capability-manifest.test.ts`'s
...
- 2026-08-04, **v4.9** — T-012 `capability-manifest-and-call-deadline` **built**. Written on
  ...
  `ARCHITECTURE.md`'s own index had been pointing at since the build. The entry for the build itself
  was found missing entirely. The index row is the authority on what landed; this is the
  ...
  against a class registry and validated at construction (D2). There are 20 manifest rows, each
  deadline number recorded with its measured envelope and the applied ceiling (D3). An absolute call
  deadline threads `resolve→fetch→throttle→safeFetch` with real cancellation and stops at the credit
  reservation (D4). `tier` collapsed four disagreeing classifications of paidness into one (D8).
  `trust` is declare-only with the validator as its sole reader (D9 slice). `CapabilityRoute.chains`
  is deleted (OQ-C). Enforcement is stated by measurement, not by intent: 4 capabilities of 20 are
  actually bounded (WI-37). Suite 1195 → 1417, zero credits spent. **One claim is narrower than v4.8
  stated** — expiry does not throw in every branch, which is OQ-T012-6, open and unresolved.
...
- 2026-08-03, **v4.8** — T-012 `capability-manifest-and-call-deadline`: **design phase, ahead of
  ...
  - **A deadline never publishes a partial as fact.** ADR-002 D4 п.5 reads "ответил хотя бы один →
    частичный результат". A deadline is nonetheless a fact about OUR availability, and the H-1
    doctrine (`packages/core/src/adapters/registry.ts:659-671`, `* Once the moment passes, the walk refuses every source it has`) forbids dressing that as a fact about the data. Returning the
    partial would report "no entity labels" for a sanctioned address when the cause is that the paid
    source did not fit the budget. Expiry throws `CapabilityDeadlineExceededError` naming both which
  ...
  - **One ADR-002 self-contradiction and one deliberate deviation from it — both recorded rather
    ...
    document disagree, and the conflict is resolved in favour of the later and more specific one.
    The _deviation_ is D4 п.5 above: the ADR is internally consistent there and we are departing
    from it on the strength of our own H-1 doctrine. Conflating the two would misreport where the
    authority lies.
  ...
  - **Amended the same day, during Planning — OD-5 and OD-6 close what the design had left to
    ...
    cannot be verified. **OD-5:** the two contested `trust` values are fixed. First,
    `platform-explorer` = `authoritative`. D9's scale asks about the REDACTABILITY of content, not
    the operator's official status. This is also the source D6 turns merging on FIRST, so a mislabel
    would ship inside the first merge. Second, `pg-history` = `community` as a **declared
    placeholder** replaced by the per-ROW rank in T-016. It is recorded as a placeholder precisely
    so a later reader does not "correct" it. Closes OQ-T012-2 and OQ-T012-3; the T-012 open list
    drops from three items to one (`shape` for 12 of 20 capabilities). **OD-6:** the `safeFetchImpl`
    deps key IS built, rather than narrowing H3 to the limiter path. D4 п.2 is a correctness
    condition. A limiter-only test would look like it checks the invariant while checking half of
    it, the exact failure mode AC-8 was amended to forbid. The reduced option is withdrawn, not left
    as a live alternative; a cost overrun there is an escalation, not a silent scope cut.
...
- 2026-08-02, **v4.7** — TASK-011 `single-tool-registry`: **the tool inventory became data.**
  ...
  - **One registry, many readers.** `src/tools/registry.ts` holds the mechanism (`ToolSpec`,
    ...
    gates all became readers of that one list. Together, `needs` + a runtime projection make least
    privilege a **fact** rather than a convention — a free tool physically cannot reach the budget
    store.
  ...
  - **Four adversarial cycles, 0 CRITICAL / 0 HIGH, verdict WARNING.** The recurring defect was not
    ...
    names were fixed (the task's own headline defect, half of it surviving all four cycles). Also
    caught: a gate written against a class that let that class through, and a repair checklist that
    had drifted into three disagreeing copies. Numbers, mutations and the un-reviewed surface:
...
- 2026-07-29, **v4.5** — TASK-009 `btc-supply-independent-verification` (research track A-3): the
  ...
  - **Checking supply against the halving formula is a tautology** (§3.2). It matched bit-exactly at
    both probed heights and will keep matching while the vendor computes it the same way. The only
    independently refutable value is the block **height**, so the cross-check compares heights
...
- 2026-07-27, **v4.4** — TASK-007 `defillama-dex-volumes`: the free DEX-volume tier (research track
  ...
  - **Coverage for `dex.volume.history` is a generated vendor list, not `vendors.defillama`**
    ...
    chains; the DEX-volume dataset covers 287, of which 274 are ours. A non-null predicate over
    that column alone would have advertised the capability on 184 chains that have no such data —
    TASK-006's H-1 defect class, repeated verbatim. `DEFILLAMA_DEX_CHAINS` is generated from
    recorded raw evidence and committed, on the same doctrine as `gen-nansen-coverage.ts`.
...
- 2026-07-27, **v4.3** — editorial pass over the whole document: index plus all ten section files
  ...
  - **Statuses finalized:** TASK-006 is delivered, not "the current task"; nothing is pending the
    ...
    OQ-1…OQ-5). Each is kept with the reason that closed it.
  - **Facts corrected against the code:** our registry is **458** chains, not the vendors' 461. Ten
    adapters and ten tools where the text still said nine and four/five. `tools/list` returns 10.
    The optional env surface is 11 keys, not 4; the monorepo tree now matches the repository.
...
- 2026-07-27, **v4.2** — two adversarial review cycles over TASK-006, plus closure of the
  ...
  - **Critical (C-1).** Nansen's paid routes opened on 7 chains of the `other` family, where
    ...
    its own `argsHash`, so the cache is no defence). Two case variants of one address were two
...
- 2026-07-26, **v4.1** — TASK-006 IMPLEMENTED (tasks 006-1…006-10). Test suite **687** (core 515 +
  ...
  - **Defects found during implementation and fixed.**
    - (a) `deps.data ?? registryData` — `null ?? x` returns `x`, so an explicit `{data:null}` was
      silently replaced by the production registry. A test intended to run on synthetic data would
      have run on production data and passed there.
...
- 2026-07-26, **v4.0** — TASK-006 architecture phase (`universal-chain-registry`, R-48…R-60): a
  ...
  - The old `z.enum(['ethereum','solana','dash'])` splits into **two** schemas: `ChainSchema`
    ...
    there into the cache key, producing two entries for one request. On paid routes that is a
    money defect.
  - The coverage matrix (`src/chain/coverage.ts`) is **derived** from
    `routes × adapter.chainSupport()`, where `chainSupport` is a predicate over `ChainInfo` rather
    than a list. A list would have to be kept in sync with the registry; a predicate cannot drift
    from it.
  ...
  - Two new free tools: `onchain_list_chains` (discovery, zero network calls, mandatory `limit` +
    `total`) and `onchain_chain_tvl`. Without mandatory `limit` + `total`, a tool built to save 8.7k
    schema tokens would spend more on its first call. `onchain_chain_tvl` is separate from
    `onchain_protocol_tvl`: a chain is not a protocol, a different source and a different contract.
  - **The only non-trivial security risk of the task — §7.2.1:** multichain RPC needs a host per
    ...
    field where autofill is banned. A chain without `rpcHosts` is honestly uncovered rather than
    failing at runtime.
  - Live probing on 2026-07-26
    ...
    is 235, on normalized names 255. Fuzzy merges must therefore land in a separate section of the
    diff report and be confirmed by a human.
...
- 2026-07-23, **v3.0** — TASK-005 architecture phase (`m2-alpha-paid`, R-29…R-47): the first paid
  ...
  - Review found, and the architecture fixed, 2 critical defects in the first draft. **C-1:**
    ...
    from `Date.now()` at reconciliation instead of being pinned at reservation. A cross-midnight
    response therefore wrote a negative delta into someone else's day bucket. Corrected to
    "`dayBucketMs` is pinned once on entry to the gate and passed down the whole call chain". Plus
    M-1: the `BudgetStore` interface is now explicit, with the ceiling deliberately NOT in it — a
    documented deviation from the literal R-35, see §3.2. M-2: `SqliteBudgetStore` upserts
    `providers` itself rather than relying on `SqliteCacheStore`'s bootstrap. M-3: the cross-process
    contract is `BEGIN IMMEDIATE` plus an explicit `timeout`, not the `DEFERRED` default.
...
- 2026-07-23, **v2.2.1** — CoinGecko Pro contour fix. Any key was previously sent as the
  ...
  `COINGECKO_PRO_API_KEY` is introduced (→ `pro-api.coingecko.com` + `x-cg-pro-api-key`) — adapter
  `coingecko`, `EnvSchema`, `.env.example`, +3 contour tests. Pro wins when both keys are set. Key
  formats are identical across tiers, so the contour is declared by a variable rather than sniffed.
  Test suite — **287** (212 core + 75 mcp-server). Same date: the document's Index-Mode split
...
- 2026-07-23, **v2.2** — synchronization with the actual code after adversarial cycles 1–3 (14+8+1
  findings, commits 8d3ea79/066cce6/8a602cc) and a polish round (61f3ab2, 6 fixes + RF-1). In
  `CapabilityRegistry.resolve()`, a cache failure is now **best-effort** and never produces
  `CapabilityUnavailableError`. For `safeFetch` — timeout (`AbortSignal.timeout`, 15 s),
  Content-Length cap 10 MB, https check on the original URL **and** on redirects,
  `Authorization`/api-key headers stripped on a cross-host redirect. For the rate limiter — a
  concurrency-safe synchronous token bucket (negative backlog, no promise chains). Plus a typed
  reject on `refillPerSec<=0` and a 30 s fairness cap (with a token refund). For the `pg-history`
  client — `pool.on('error')`, `connectionTimeoutMillis=10000`/`max=3`, sanitization of **all**
  failure paths including the `Pool` constructor. For `onchain_get_token` — capability
  `token.metadata` → `token.price` (TTL 60 s, not 3600 s). For `address`/`protocolSlug` — explicit
  `.max()` bounds. Now `onchain_new_pairs` materializes the default `limit` before the cache key.
  Adapters hardened (rpc-evm hex regex, rpc-solana safe-integer lamports, dexscreener skip-and-log,
  defillama finite/non-negative tvl, a shared `stringify-truncated.ts`). In the cache DB — prepared
  statements, sweep every 50 writes, leak-safe constructor, honest `ageMs` on LRU promotion. The
  stale `isError` wording corrected (SDK 1.29 intercepts any throw from a handler, not only zod
  validation). Test suite — **284** (209 core + 75 mcp-server).
