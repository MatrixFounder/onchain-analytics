...
- Rate-limit: `packages/core/src/net/rate-limit.ts:102-140`, `MAX_WAIT_MS = 30_000` (`:32`). У Dune два класса лимита (low 15 rpm + high 40 rpm, комбинированно 55), а наша регистрация держит один бакет на адаптер (`types.ts:68-73`) — либо берём нижний, либо разносим на две регистрации.
