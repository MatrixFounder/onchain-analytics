...
(`system-architecture-chain-normalization.md:62` показывает `chainSupport?(chain: ChainInfo)` без параметра `capability`,
который есть в коде; `:86-90` показывает `CapabilityRoute` без поля политики; `:115-174` всё ещё
