...
Сегодня `toolSpecs` содержит 20 записей. Задача 014-32b добавляет оба тула — `onchain_pool_info` и `onchain_token_pools`, — после чего их 22. Тест обходит реестр, число тулов в нём не зашито.
