...
resolved_by: 'controlled experiment (probe:sweep-contention) + a gate run, both refuting the premise'
---
...
>
> ## ЭТА ЗАПИСЬ ОПРОВЕРГНУТА СВОИМ ЖЕ ЭКСПЕРИМЕНТОМ в тот же день. Читайте это первым.
>
> **Центральное утверждение — «падает внутри обхода и проходит вне его» — не выдержало контрольного
> сравнения.** `packages/core/scripts/probe-defillama-sweep-contention.ts`, три плеча, каждый вызов
> под настоящим 15-секундным дедлайном способности:
>
> | плечо | результат | худший вызов |
> | :-- | :-- | :-- |
> | A — обход в одиночку | **11 из 11 ok** | 580 мс |
> | B — обход при брошенных загрузках в полёте | **11 из 11 ok** | 330 мс |
> | A′ — обход в одиночку снова | **11 из 11 ok** | 418 мс |
>
> Эффекта тесноты нет: плечо B оказалось даже быстрее контролей. А следом полный прогон гейта — тот
> самый широкий обход, который якобы и вызывает отказ, — взял **все 11 строк** `dex.volume.history`
> за 89–1730 мс, на канале, который гейт сам назвал стабильным (42 пробы, 0 отказов, медианный
> connect 14–17 мс). В том же прогоне `L-24/dex.volume.history` и `L-24/protocol.tvl.history` дали
> **0 из 11** каждая: вендор восстановился целиком.
>
> **Где была ошибка рассуждения, а не измерения.** Три замера, на которые опиралась запись, взяты в
> РАЗНОЕ время: прогон гейта, а потом — минутами позже — `curl` и одиночный вызов тула. Это «упало
> тогда, прошло потом», а не «падает в обходе, проходит вне его». Одновременного сравнения не было
> ни одного, и именно его отсутствие я записал как установленный факт. Формулировка «What IS
> established» ниже была неверна в тот момент, когда её писали.
>
> **Что эксперимент НЕ доказал, и это важно не меньше.** Нагрузка, ради которой он ставился, не
> состоялась: из четырёх медленных документов к концу плеча B в полёте остался **один** — остальные
> отказали быстро, а не качались. То есть теснота проверена только в условиях «вендор здоров», и
> утверждать, что её нет при по-настоящему медленном вендоре, эти данные не позволяют. Отрицательный
> результат здесь узкий, и расширять его нельзя.
>
> **Урок, ради которого запись сохранена (и он единственная причина не удалять её).** Эксперимент про
> ПЕРЕХОДНЫЙ процесс надо ставить ВО ВРЕМЯ этого процесса. Я поставил его после того, как окно
> закрылось, и получил ответ на вопрос, которого не задавал. Если картина повторится — прогонять
> `probe:sweep-contention` в тот же час, пока строки красные, а не следующим утром.
>
> Строки возвращены под [L-24](l-24-defillamas-two-per-call-history-routes-answer-erratically.md):
> объяснения «origin вендора деградирует окнами» достаточно, и второй записи для тех же строк не
> нужно. Severity понижена SEV-2 → SEV-4: дефекта не установлено, сохранён урок.
