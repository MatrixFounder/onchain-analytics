...
(`system-architecture.md:354` показывает `chainSupport?(chain: ChainInfo)` без параметра `capability`,
который есть в коде; `:378-382` показывает `CapabilityRoute` без поля политики; `:407-466` всё ещё
