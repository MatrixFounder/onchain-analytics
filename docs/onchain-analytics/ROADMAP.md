...
**Четырнадцать тулов**, в порядке публикации:
...
- **Now:** **трек T-010…T-015 — конфигурируемый роутинг и сетевой интерфейс** — четыре этапа из шести сданы
  (T-010…T-013 ✅); **следующий — T-014** (HTTP + токен + периметр + профили + общий лимитер). Решения владельца 2026-07-31,
  ...
  | **T-014** | HTTP + токен + периметр + профили + общий лимитер — **полный объём** (решение владельца) | ≈2 нед |
  | **T-015** | Тарификация фазы 0 (`BillingStore`, прайс-лист, идемпотентность) + замер расхода Blockscout | 1–1.5 нед |
