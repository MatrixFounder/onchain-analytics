...
| **Источники**           | TASK.md §1 п.4 / UC-8 / §6; [system-architecture.md](../architectures/system-architecture.md) §3.2 |
| **Живые кредиты**       | **≈16 из потолка ≤30** — см. бюджет ниже                                                           |
| **GOLDEN_TEST_SHA**     | `c514c8ac…d747cf8` — базовый, см. «Golden-test SHA» ниже                                           |

## Golden-test SHA

Базовый (pre-005-7) хеш `packages/core/test/nansen.contract.test.ts` — снят на acceptance
005-4 Phase 2, 2026-07-24, ещё на spec-derived фикстурах:

```
c514c8ac129a568aa2e8330458d2a08928d5c75b4fd76c0999e2d1d6cd747cf8
```

Снимается командой `shasum -a 256 packages/core/test/nansen.contract.test.ts`. Смысл — поймать
**тихую подгонку теста под фикстуру**: любое изменение ассертов обязано быть залогировано, а не
просто «оказаться» в коммите.

> [!WARNING]
> Цепочка хешей **разорвана между последним залогированным значением и коммитом `4c51126`** — см.
> [RF-2](../issues/rf-2-m2-evidence-records-drifted-from-the-shipped-commit.md) и разметку в
> evidence-файле. Начиная с цикла 4 хеш снимается с **закоммиченного блоба**
> (`git show <sha>:<path> | shasum -a 256`), а не с рабочего дерева — именно сверка только с рабочим
> деревом и делала разрыв невидимым.
