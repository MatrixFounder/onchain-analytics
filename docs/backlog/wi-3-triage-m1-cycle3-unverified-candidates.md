...
resolved_by: triage completed 2026-07-23; descendants verified 2026-07-28
---

# Triage M1 adversarial cycle-3 unverified candidates (10 MINOR + 4 bikeshed)

> **DONE — the deliverable was the triage, and it happened on 2026-07-23** (6 fixed, 3 deferred to
> M2, 5 accepted as documented). Verified 2026-07-28 that the three M2 deferrals actually landed:
> singleflight coalescing → `packages/core/src/adapters/nansen/singleflight.ts` (task 005-5);
> chunked-response size cap → `safeFetch`'s `capResponseStream` (TASK-007 task 007-3);
> solana exact lamports → tracked separately as **WI-8**, now also closed. Nothing here is
> outstanding; the entry stayed `open` only because no one flipped it after the triage.
