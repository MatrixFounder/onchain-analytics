...
- **Дата:** 2026-06-30 · **Обновлён:** 2026-07-27 (**TASK-006 сдан и дважды отревьюен, реестр известных проблем пуст**; ранее 2026-07-24 — **M2 выполнен**; ранее — **M1 выполнен** 2026-07-23; ранее — **M0 выполнен** 2026-07-22; ранее 2026-07-20 — снапшоттер Dash Platform + privacy-метрики, пометки *(2026-07-20)*) · **Статус:** Active (M0 ✅ · M1 ✅ · M2 ✅ → M3)
...
| **M2** Alpha-слой (paid) ✅ *(2026-07-24)* | «куда идут умные деньги» | 1–2 нед | Nansen (free-план) | smart-money + метки + budget-guard |
...
## M2 — Alpha-слой, платный (≈1–2 нед) — ✅ ВЫПОЛНЕН 2026-07-24

> ***(2026-07-24) Итог:*** все три exit-критерия закрыты, **41 кредит потрачен** (100 → 59), каждый
> транш согласован владельцем. Тест-сьют **287 → 492** без единой регрессии M1; lint/format/
> typecheck/build/smoke-dist зелёные; офлайн-прогон (сеть заблокирована) доказывает, что после
> записи фикстур сьют не делает ни одного исходящего вызова.
>
> **Поправки к исходным допущениям ROADMAP (все подтверждены живыми пробниками):** аккаунт на плане
> **`free` со 100 кредитами**, а не Pro $49 — движок спроектирован под оба и переключается по живому
> `/account` без правки кода; MCP-сервер Nansen отдаёт **37 tools**, а не 24; REST — **75 путей**.
> Интеграция сделана через **REST, а не через MCP-сервер вендора** (несколько его tools возвращают
> markdown, непригодный для канонизации, и проксирование обошло бы наш кеш/бюджет/SSRF-гейт).
> `x-credit-cost` задокументирован **у всех 74 операций** → `costOf()` читает таблицу,
> сгенерированную из закоммиченной спеки, и не тратит кредиты на выяснение цен. Метки переисточены
> с `/profiler/address/labels` (100cr = весь баланс) на `/search/general` (0cr) + `/tgm/holders` (5cr).
> **Bitquery отложен** (YAGNI, ключа нет).
>
> **Ключевая находка (DF-1, 30cr на диагностику):** `/smart-money/netflow` возвращал well-formed
> пустой результат по двум нашим ошибкам — не выставлялись `include_stablecoins`/`include_native_tokens`
> (у обеих default `false`), и фильтр `token_address` у этого эндпоинта **регистрозависим**, тогда как
> соседние `/tgm/*` принимают EIP-55. Класс «молчаливый пустой ответ»: HTTP 200, оплачено, ошибок нет.
>
> Адверсариальная ревизия: **3 цикла**, три критика (логика/безопасность/производительность), 20+
> находок исправлено — в т.ч. пустой заголовок кредитов обнулял бюджет-леджер; фантомные резервации
> при падении под-вызова; тело ответа вендора попадало в контекст модели без вычистки ключа;
> `.max()`-лимиты, поставленные на первом цикле, сами открыли утечку кредитов (бросок после оплаты)
> и были переделаны на обрезку. **`Q-2` закрыт** (2026-07-25): `NANSEN_DAILY_CREDIT_CAP` получил три
> состояния — не задан → потолок выводится сам (`max(30, 25% остатка)`, фиксируется на суточный
> бакет), число → явный потолок оператора, `off` → самоограничение выключено. Защита включена из
> коробки. Остаточный вопрос — `Q-1` (by-design).
>
> **Четвёртый цикл (`/vdd-multi`, 2026-07-25)**, три критика + аудит полноты по RTM: 5 дефектов кода
> исправлено (адрес Solana портился принудительным `toLowerCase()` — DF-1-фикс проверялся живьём
> только на EVM; выводимый потолок считался от ПОСЛЕ-расходного баланса и после перезапуска сервера
> мог запереть весь платный слой на сутки; `credits_remaining` принимался без доменной валидации,
> т.е. `Infinity` снимал потолок до конца бакета; `null`-элемент в массиве вендора ронял уже
> оплаченный ответ; сбой записи в леджер уничтожал оплаченный результат). НЕ закрыто и не заявлено
> как сделанное: лимит скорости (суточный потолок не держит всплеск), отрицательное кеширование
> пустых ответов, бесплатный тариф `entity.labels` (0cr) гейт структурно не может отклонить,
> `/tgm/token-information` оплачивается 1cr на вызов и не читается.
> Детали: [task-005-7 evidence](raw/task-005-7-live-verification-evidence.md), [KNOWN_ISSUES](../KNOWN_ISSUES.md).
...
| M2–M4 | **$0 на M2** — уложились в free-план; далее см. ниже |
| M5 | + комиссии бирж/газ (CDP-ключ бесплатен; торгуете своими средствами) |
| M6 | + хостинг ($5–20 VPS) + опц. Redis/Postgres managed |

**M2–M4 подробно.** Сборка M2 стоила **41 кредит из 100** на free-плане, то есть $0 живыми
деньгами. Дальше по потребности:

- **~$49/мес Nansen Pro** — когда упрёмся в объём (free-плана хватает на разработку, но не на
  регулярный снапшоттинг);
- **Bitquery** — опционально, по запросу; решение об интеграции ещё не принято.

> Намеренно избегаем: Dune Sim (**sunset 01.08.2026**), Allium (~$5K+/мес enterprise), Glassnode
> Professional (custom $) — до явной потребности.
...
- **TASK-006 (вне M-нумерации, по запросу владельца перед M3): ✅ выполнен 2026-07-26, отревьюен дважды 2026-07-26/27.** Сеть стала данными: реестр 458 сетей как build-артефакт, производная матрица покрытия, `chain` — открытая строка + `onchain_list_chains` (≈7.8k токенов схемы сэкономлено на каждом запросе к модели). Два цикла `/vdd-multi` дали 1 Critical + 6 High, все исправлены; попутно закрыты три последние записи реестра — **SEC-1** (тормоз по скорости расхода), **Q-3** (лимит по числу вызовов — единственный знаменатель, видящий вызов ценой 0 кредитов), **RF-2** (механизм провенанса вместо рукописного хеша). Тест-сьют 687 → **796**, оффлайн-прогон зелёный, живых кредитов 0. Детали: [TASK.md](../TASK.md), отчёты [цикл 5](../reviews/task-006-vdd-multi.md) / [цикл 6](../reviews/task-006-vdd-multi-cycle6.md).
