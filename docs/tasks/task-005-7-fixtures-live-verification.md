...
1. **TC-VERIFY-01 (R-44, бюджет):** `remaining_before − remaining_after === 16` (±допустимое
   ...
   → ❌ **НЕ ВЫПОЛНЕН: фактический расход 41cr.** Оценка 16cr не выдержала — первые три живые
   попытки `/smart-money/netflow` вернули пустой результат из-за двух наших дефектов запроса
   ([DF-1](../issues/df-1-nansen-smart-money-netflow-empty-for-base-pair-tokens.md)), диагностика и
   перезапись стоили ещё 25cr. Каждый транш авторизован владельцем отдельно. Формулировка гейта
   оставлена как есть намеренно — переписать её задним числом означало бы стереть факт.
...
3. **TC-CONTRACT-01…05:** golden-тесты 005-4 зелёные на **реальных** фикстурах.
   → ✅ выполнены. ⚠️ Две поправки к формулировке (vdd-multi цикл 4, G-10):
   (а) диапазон здесь изначально был записан как «03…07» — **`TC-CONTRACT-06`/`07` не существуют**
   и никогда не существовали, реальный диапазон `01…05`;
   (б) «без изменения ассертов» не выдержано и не могло быть: `TC-CONTRACT-01/02/04` переписаны под
   реальные записанные значения, а `TC-CONTRACT-01` дополнительно возвращён из ошибочной
   `toThrow()`-формы (она кодифицировала наш собственный дефект как ожидаемое поведение) в проверку
   заполненного результата. Все правки залогированы в шапке самого теста и в §6 evidence-файла.
4. **TC-VERIFY-08 (R-44, offline):** `pnpm test` при отключённой сети — зелёный, 0 исходящих вызовов.
   → ✅ выполнен, и **перепроверен против фактически отгруженного дерева** (519/519, 0 исходящих).
   Первая запись гейта (453/453) относилась к предку коммита — расхождение зафиксировано как
   [RF-2](../issues/rf-2-m2-evidence-records-drifted-from-the-shipped-commit.md) и закрыто.
5. **TC-VERIFY-09 (evidence):** ни один `*.evidence.md` не содержит строки `provenance: spec-derived`
   и не содержит значения ключа.
   → ⚠️ **выполнен наполовину.** Значения ключа нет нигде — это держится. Но **три** evidence-файла
   до сих пор несут `provenance: spec-derived`: `account.pro` (нет живой Pro-подписки),
   `search-general.empty` и `tgm-holders.empty-labels` (нет воспроизводимой живой формы «0
   результатов»). Все три вне скоупа этой задачи по её же тексту, но сам TC написан как безусловный
   и потому формально красный. Записано, а не подогнано.
