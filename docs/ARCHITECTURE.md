...
| **Updated**       | 2026-08-05, **v4.9.3** — T-012 BUILT, hardened over three adversarial cycles plus a fix pass, and OQ-T012-6 resolved (ten tasks, suite 1195 → 1473, zero credits spent). The answer-sufficiency policy is a serialisable descriptor resolved against a class registry and validated at construction (D2); capabilities carry a manifest of 20 rows, each deadline number recorded with its measured envelope and the applied ceiling (D3); an absolute call deadline threads resolve→fetch→throttle→safeFetch with real cancellation, and stops at the credit reservation (D4); `tier` collapsed four disagreeing classifications of paidness into one (D8); `trust` is declared with the validator as its only reader (D9 slice); `CapabilityRoute.chains` is deleted (OQ-C). **Narrower than v4.8 claimed:** expiry does not throw in every branch — a walk where every source was entered and every one answered returns that answer past the ceiling, marked with `_meta.timing.overrunMs`. Owner-resolved 2026-08-05 (OQ-T012-6): the ceiling bounds SPENDING, not the moment of delivery. Full changelog: [architectures/version-history.md](architectures/version-history.md) |
...
narrowed to the limiter path). Everything else is recorded as resolved, with the reasoning that keeps
it closed.
