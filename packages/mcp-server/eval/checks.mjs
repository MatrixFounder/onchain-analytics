 ...
 * `onchain_token_holders` was wired into `CAPABILITY_TOOLS`, so the live eval called it on every
    ...
    return {
      ...
      problems: [
        `NO CHECK WIRED for ${tool} — the eval called it and cannot tell a good answer from an ` +
          'empty one. Add an entry to `checks` in eval/checks.mjs, or stop calling the tool.',
      ],
    };
  }
