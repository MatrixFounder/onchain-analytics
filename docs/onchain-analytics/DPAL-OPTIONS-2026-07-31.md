...
| «Первый найденный» + FallbackOnly | Реализовано, причём с различением, которого в списке Strategy нет: «все ответили, ни у кого нет» ≠ «кого-то не удалось спросить» (`registry.ts:440-455`) |
...
5. **Превышение — типизированное событие.** Ни один источник не ответил → ошибка. Ответил хотя бы один
   ...
   действует у `hadFailure` / `unsatisfying` (`registry.ts:440-455`). До T-013 это академично, после
