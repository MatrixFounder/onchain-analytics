...
**2026-09-02 · `onchain-engine-db` · удалена временная база `wi62_restore_check` (`DROP DATABASE`,
по явному подтверждению владельца).** Осмотр перед удалением опроверг то, ради чего её берегли:
она несла не тринадцать таблиц движка, а ТРИ снапшоттерских — `assets`, `metrics`, `snapshots`, —
и все пустые. Копии `api_tokens.token_hash` в ней не было. Это стенд, на котором репетировалась
охрана миграции 006: файл удаления требует ровно эти три таблицы в схеме `onchain`, чтобы отличить
старый контейнер от нового. После удаления на контейнере одна база `postgres`; живая схема
проверена — 13 таблиц, 1 триггер, `api_tokens` 10 строк.

**2026-09-02 · инстанс n8n · установлены две фоновые работы, обе по согласию владельца.**
`onchain-billing-reconcile` создан (id `VZZXwYTKSOfQXL1k`, 10 узлов, каждые 15 минут) и активирован
ПОСЛЕ проверки: `n8n_validate_workflow` — 0 ошибок, 0 предупреждений, 9 связей из 9; разводка
прочитана отдельно, потому что валидация о ней не говорит. `onchain-retention` обновлён с 16 узлов
до 19; сверка перед заменой показала, что репозиторий есть строгое надмножество живого воркфлоу —
исчезающих узлов ноль, — а снимок инстанса сохранён до записи.

Подтверждение работой, а не активацией: `onchain.retention_runs` несёт строки
`client_usage.reconcile_expired` за `19:45:28` и `20:00:28` UTC, обе `outcome=ok`,
`rows_affected=0`. Прогон, не нашедший ни одной зависшей строки, всё равно пишет свою — это R-14.3,
и проверялось именно это. Исполнений в списке n8n при этом НЕТ и быть не должно: настройка
`saveDataSuccessExecution: none` успешные прогоны не сохраняет, поэтому свидетельством служит база,
а не журнал исполнений. Второй прогон (`20:00:28`) позже отметки `updatedAt` воркфлоу
(`19:56:42`) — то есть отработал уже переименованный и инвертированный граф.
