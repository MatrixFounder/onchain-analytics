...
> и были переделаны на обрезку. Остаточные вопросы — `Q-1` (by-design) и **`Q-2` — решение владельца:
> `NANSEN_DAILY_CREDIT_CAP` не имеет дефолта, т.е. в стоковой установке самоограничения бюджета нет**.
