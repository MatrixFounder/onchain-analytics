...
ADR-002 D2 specifies: if a construction-time audit of all 26 routes ever finds one that genuinely
...
| ------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `any`               | always `true` (today's implicit default)                        | 25 of 26 routes, which carry no policy today                                                                                                                                                                                                                                                                |
...
**13 of the 26 routes** (measured from`providers.config.ts`; the other 8 are the five
...
**The route table is NOT reproduced here.** `providers.config.ts` holds **26 routes** over 25
...
| `dexscreener`       | `pairs.active`, `pool.info`                                    | REST (`fetch`)                                                                                                       | none (keyless)                                                                                                         | R-6 Must requires both; `pool.info` has no tool consumer yet; exact endpoint confirmed when the fixture is recorded (R-22), §11; **live**     |
...
- **The tool inventory is data, not prose (TASK-011, [ADR-002](../onchain-analytics/ADR-002-configurable-routing.md)
  ...
  it would silently drop four titles from `tools/list`. **Today all 19 carry one** (measured while
...
spec.servedCapabilities`.
  - 🔴 **Least privilege stays a RUNTIME fact, not a type-level promise.** Today `server.ts` hands
    ...
    to all nineteen would replace that with self-restraint — and self-restraint is weak here,
...
- **`packages/mcp-server/test/e2e.stdio.test.ts`** (spawn; the mechanism is unchanged from M0) —
  ...
  exactly the **nineteen** tools **derived from `toolSpecs`** — `toHaveLength(expected.length)` at
...
flowchart TB
  ...
  TOOLS["mcp-server/src/tools/*.ts — 13<br/>ping + get-token + wallet-balances<br/>+ active-pairs + protocol-tvl + M2/TASK-006 tools<br/>+ dex-volume + token-holders + chain-supply"]
