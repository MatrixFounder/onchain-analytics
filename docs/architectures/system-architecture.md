...
расширение станет вектором расхода денег (NFR TASK §7). Это же требование закрывает часть
поверхности **SEC-1** (`open`: дневной потолок не ограничивает всплеск) — не сам velocity-guard,
но снятие с платного пути целого класса заведомо бесполезных вызовов.
