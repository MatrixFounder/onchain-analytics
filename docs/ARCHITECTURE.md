...
- **Интерфейсы:**
  - _Inbound:_ stdio (stdin/stdout), протокол MCP (JSON-RPC), клиент — Claude Code или любой
    MCP-host, использующий `StdioClientTransport`.
  - _Outbound:_ нет (M0 не делает исходящих сетевых вызовов — нет провайдеров).
